...
PIPELINE_GATES = [
    ("discover", "Discover", "Public Robota.ua search/listing URLs only"),
    ...
    ("final_gated_apply_manual_handoff", "Final Gated Apply / Manual Handoff", "No Robota.ua submit automation"),
...
def _is_robotaua_vacancy_href(href: str) -> bool:
    return bool(re.search(r"/vacancy\d+", href, re.IGNORECASE))
...
class _VacancyAnchorCollector(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if not _is_robotaua_vacancy_href(href):
            return
...
class RobotaUaAdapter(JobPlatformAdapter):
    ...
    def normalize_vacancy(self, raw: dict) -> VacancyObservation:
        ...
        vacancy_id = _robotaua_vacancy_id(source_url)
        if vacancy_id:
            raw_payload.setdefault("robotaua_vacancy_id", vacancy_id)
    ...
    def extract_public_vacancies(
        ...
        """Extract Robota.ua observations from public listing/detail markup."""
...
def robotaua_observation_score(observation: VacancyObservation | dict[str, Any]) -> int:
...
def persist_robotaua_observation(
    observation: VacancyObservation,
    db_path: Path = DEFAULT_DB_PATH,
) -> int:
    if observation.source_site != "robotaua":
        raise ValueError(f"expected robotaua observation, got {observation.source_site!r}")
    return upsert_job(observation, score=robotaua_observation_score(observation), db_path=db_path)


def create_robotaua_review_draft(
    observation: VacancyObservation,
    job_id: int,
    ...
    append_event(
        ...
        "needs_owner_review",
        {
            ...
            "reason": "Robota.ua adapter has no submit automation; owner must review exact vacancy/message/resume.",
...
def _robotaua_rows(db_path: Path, limit: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    rows = fetch_progress_rows(db_path, limit=limit)
    jobs = [row for row in rows["jobs"] if row.get("platform") == "robotaua"]
...
def _gate_status(gate: str, jobs: list[dict[str, Any]], outreach: list[dict[str, Any]], events: list[dict[str, Any]]) -> str:
    if gate == "discover":
        return "active" if any(row.get("event_type") == "robotaua_public_fetch_complete" for row in events) else "ready"
    if gate == "normalize":
...
def build_robotaua_progress_snapshot(db_path: Path = DEFAULT_DB_PATH, limit: int = 100) -> ProgressSnapshot:
    jobs, outreach, events = _robotaua_rows(db_path, limit)
    ...
    for index, (gate_id, label, description) in enumerate(PIPELINE_GATES):
        ...
        if index:
            ...
            edges.append(
                ProgressEdge(
                    source=f"robotaua:{previous}",
                    target=f"robotaua:{gate_id}",
                    label="next",
    ...
    for job in jobs:
        ...
        edges.append(ProgressEdge(source="robotaua:normalize", target=f"robotaua:job:{job_id}", label="observed", status="seen"))
        if int(job.get("score") or 0):
            edges.append(ProgressEdge(source=f"robotaua:job:{job_id}", target="robotaua:score", label="scored", status="complete"))
    ...
    for draft in outreach:
        ...
        nodes.append(
            ProgressNode(
                id=f"robotaua:outreach:{outreach_id}",
                label=f"Robota.ua Draft #{outreach_id}",
                kind="outreach_draft",
                status=status,
                data={
                    "job_id": job_id,
        ...
        edges.append(ProgressEdge(source=f"robotaua:job:{job_id}", target=f"robotaua:outreach:{outreach_id}", label="draft", status="active"))
        edges.append(
            ProgressEdge(
                source=f"robotaua:outreach:{outreach_id}",
                target="robotaua:review_approval",
                label="review gate",
                status=status,
            )
        )
        edges.append(
            ProgressEdge(
                source=f"robotaua:outreach:{outreach_id}",
                target="robotaua:final_gated_apply_manual_handoff",
    ...
    return ProgressSnapshot(
        schema="job.robotaua_progress_snapshot.langgraph.v0",
